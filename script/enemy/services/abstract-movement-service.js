...
import { GUESS_SEARCH, INVESTIGATE, LOST, NO_OFFENCE } from '../util/enemy-constants.js'

export class AbstractMovementService {
    constructor(enemy) {
        this.enemy = enemy
        this.offenceService = new AbstractMovementService(enemy)
    }

    move2Destination() {
        if ( this.offenceService.collidePlayer() ) return
        ...
        this.enemy.htmlTag.style.left = `${this.x}px`
        this.enemy.htmlTag.style.top = `${this.y}px`
    }
